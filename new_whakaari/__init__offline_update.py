...
class TremorData(object):
    ...
    def _assess(self):
        ...
        if not self.exists:
            t0 = datetime(2019,11,25)    # start date of new data to be uploaded
            t1 = datetime(2019,11,30)   # end data of new data to be uploaded
            self.update(t0,t1)
    ...
    def update(self, ti, tf):
        ...
        self.df = self.df.resample('1T').interpolate('linear')     # Interpolation resolution of data
...
class ForecastModel(object):
    ...
    def __init__(self, window, overlap, look_forward, ti=None, tf=None, data_streams=['rsam','mf','hf','dsar'], root=None):
        ...
        self.dtw = timedelta(minutes=self.window)
        self.dtf = timedelta(minutes=self.look_forward)
    ...
    def forecast(self, ti=None, tf=None, recalculate=False, use_model=None, n_jobs=6):
        ...
        return forecast
    def hires_forecast(self, ti, tf, recalculate=True, save=None, root=None, nztimezone=True,
    ...
    def _plot_hires_forecast(self, ys, save, threshold=0.75, nztimezone=False):
        ...
        plt.savefig('/Users/bst/Documents/All/PhD/Data/Codes/RF_Dempsey/plots/test/Aug2013.png', dpi = 400)
    import os
    if not os.path.isdir('/Users/bst/Documents/All/PhD/Data/Codes/RF_Dempsey/plots/test'):
        os.makedirs('/Users/bst/Documents/All/PhD/Data/Codes/RF_Dempsey/plots/test')
...
def get_data_for_day(i,t0):
    ...
    #data_streams = [[1.2, 5], [1.2, 25]]
    names = ['rsam','mf','hf']
    #names = ['RSAM_low', 'RSAM_high']
    ...
    t0s=t0+i*daysec
    t0f=str(t0s)
    WIZ = read('/Users/bst/Documents/All/PhD/Data/SeismicData/Whakaari_2019/'+t0f[0:23]+'.mseed')
    WIZ.remove_sensitivity(inventory=site)
    ...
    ti = tiday+int(np.round((ti-tiday)/60))*60  # (both numbers) data windows in seconds
    N = 60*100                                  # data windows in seconds * 100
    ...
    time = [(ti+j*60).datetime for j in range(datas.shape[1])]              # adapt data window size (in seconds)
