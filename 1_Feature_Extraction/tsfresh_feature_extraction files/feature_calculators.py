...
from scipy.signal import cwt, find_peaks_cwt, ricker, welch, argrelmax
...
def agg_autocorrelation(x, param):
    ...
    if np.abs(var) < 10**-10 or n == 1:
        ...
        a = acf(x, fft=True, nlags=max_maxlag)[1:]
...
@set_property("fctype", "combiner")
@set_property("not_in_first", True)
def multi_ACF_lag_stats(x, param):
    """
    Calculates the autocorrelation within 10-min chunks (parameters = range(5)) in the second FE stage for 1h windows.
    """
    if type(x) is pd.Series:
        x = x.values

    n_chunks    = int(param[-1]["chunk_no"])+1
    chunk_len   = int(len(x)/(n_chunks))
    max_lag     = int(chunk_len/2)-1

    def pacf_stats(x, max_lag, chunk_no, attr):
        data        = x[chunk_no*chunk_len:(chunk_no+1)*chunk_len]
        pacf_vals   = pacf(data, method="ld", nlags=max_lag)[1:]
        lag_max     = np.argmax(pacf_vals)
        acf_val     = max(pacf_vals)
        if attr == "lag_max":
            return lag_max
        if attr == "acf_val":
            return acf_val

    res = [pacf_stats(x, max_lag, config["chunk_no"], config["attr"]) for config in param]
    indices = ['chunk{}_{}'.format(config["chunk_no"], config["attr"]) for config in param]

    return zip(indices, res)


@set_property("fctype", "combiner")
def single_ACF_lag_stats(x, param):
    
    if type(x) is pd.Series:
        x = x.values
    '''
    Calculates autocorrelation value for different lags (1,2,...,len(x)/2) and returns the lag ('lag_max') with the
    maximum autocorrelation and the autocorrelation value of that lag ('val_max')
    '''

    attributes = [attr for attr in param[0]["attr"]]

    vals = []
    min_lag = 2

    for i in range(min_lag,int(len(x)/2)):
        y1 = x[:(len(x)-i)]
        y2 = x[i:]
        x_mean = np.mean(x)
        sum_product = np.sum((y1 - x_mean) * (y2 - x_mean))
        v = np.var(x)
        vals.append(sum_product / ((len(x) - i) * v))

    lag_max = np.argmax(vals) + min_lag
    val_max = max(vals)
    lag_min = np.argmin(vals) + min_lag
    val_min = min(vals)
    all_vals = {"lag_max": lag_max, "val_max": val_max, "lag_min": lag_min, "val_min": val_min}

    return [("{}".format(attr), all_vals[attr]) for attr in attributes]
...
@set_property("fctype", "simple")
@set_property("minimal", True)
def variance_coefficient(x):
    """
    Returns the variance coefficient of x
...
def dsar(x, band):
    """
    This feature calculator DSAR as the ratio between different frequency bands (1-16Hz).
    ...
    fband_h = band[1]
    freq = int(len(x)/10)
    filt_low = mean(abs(bandpass(x, *fband_l, freq)))
    filt_high = mean(abs(bandpass(x, *fband_h, freq)))

    dsar = filt_low/filt_high
    return dsar
...
def central_freqs(x, param):
    ...
    freq, pxx = welch(x, fs = len(x)/10, nperseg = len(x)/2, noverlap = 0.5)

    #look only at 1-25Hz (0.2Hz intervals in spectrogram)
    freq = freq[5:126] 
    pxx = pxx[5:126]

    peaks = [freq[i] for i in argrelmax(pxx)]
    top_freqs = heapq.nlargest(5, pxx)  #returns index of 5 central frequencies

    #FREQUENCIES
    dom_freq            = freq[list(pxx).index(np.max(pxx))]
    top2_freq           = freq[list(pxx).index(top_freqs[1])]
    top3_freq           = freq[list(pxx).index(top_freqs[2])]
    top4_freq           = freq[list(pxx).index(top_freqs[3])]
    top5_freq           = freq[list(pxx).index(top_freqs[4])]
    mean_peaks          = np.mean(peaks)
    std_peaks           = np.std(peaks)
    var_peaks           = np.var(peaks)

    n_peaks             = np.shape(peaks)[1]

    #SPECTRAL POWER
    dom_freq_power      = np.max(pxx)
    top2_freq_power     = top_freqs[1]
    top3_freq_power     = top_freqs[2]
    top4_freq_power     = top_freqs[3]
    top5_freq_power     = top_freqs[4]
    mean_peaks_power    = np.mean([pxx[i] for i in argrelmax(pxx)])
    std_peaks_power     = np.std([pxx[i] for i in argrelmax(pxx)])
    var_peaks_power     = np.var([pxx[i] for i in argrelmax(pxx)])

    all_vals = {"dom_freq": dom_freq, "top2_freq": top2_freq, "top3_freq": top3_freq, "top4_freq": top4_freq, "top5_freq": top5_freq, "mean_peaks": mean_peaks, "std_peaks": std_peaks, "var_peaks": var_peaks, 
                "n_peaks": n_peaks, "top2_freq_power": top2_freq_power, "top3_freq_power": top3_freq_power, "top4_freq_power": top4_freq_power, "top5_freq_power": top5_freq_power, "dom_freq_power": dom_freq_power, "mean_peaks_power": mean_peaks_power, "std_peaks_power": std_peaks_power, "var_peaks_power": var_peaks_power}

    params = ["dom_freq", "top2_freq", "top3_freq", "top4_freq", "top5_freq", "mean_peaks", "std_peaks", "var_peaks", "n_peaks", "dom_freq_power", "top2_freq_power", "top3_freq_power", "top4_freq_power", "top5_freq_power", "mean_peaks_power", "std_peaks_power", "var_peaks_power"]
    return zip([config for config in params], [all_vals[i] for i in params])
...
def ar_coefficient(x, param):
    ...
    for parameter_combination in param:
        ...
        if k not in calculated_ar_params:
            try:
                calculated_ar_params[k] = AR(x_as_list, lags=k).fit().params
...
def permutation_entropy(x):
    ...
    freq = int(len(x)/10)
    x_filt = bandpass(x, 1, 7, freq)

    # m=5 and L=2 (after Konstantinou et al., 2022: Perm. Entropy at Shinmoedake)
    m = 5
    L = 2
    # create list of tuples
    data = [x_filt[i:i+m] for i in range(len(x_filt)) if(i%L==0) and (len(x_filt[i:i+m])==m)]
    # convert to rank sequence
    data_ranked = [list(rankdata(data[i])) for i in range(len(data))]

    perm_entropy = ordpy.permutation_entropy(data_ranked, dx = m)
...
def shannon_entropy(x):
    # for now basically binned entropy, but filtered
    freq = int(len(x)/10)
    x_filt = bandpass(x, 1, 16, freq)

    if not isinstance(x_filt, (np.ndarray, pd.Series)):
        x = np.asarray(x_filt)
