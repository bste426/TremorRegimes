...
class ComprehensiveFCParameters(PickableSettings):
    ...
    def __init__(self):
        ...
        for name, func in feature_calculators.__dict__.items():
            if (callable(func) and hasattr(func, "dependency_available") and getattr(func, "dependency_available") is False):
...
class FirstFCParameters(PickableSettings):
    """
    ...
    that some of the feature calculators are not returned in the first FE stage
    (10 sec from raw data).
    """
    ...
    def __init__(self):
        ...
        for name, func in feature_calculators.__dict__.items():
            if callable(func) and not hasattr(func, "not_in_first") and hasattr(func, "fctype"):
                name_to_param[name] = None
        ...
        name_to_param.update({
            ...
            "cwt_coefficients": [{"widths": width, "coeff": coeff, "w": w} for
                                 width in [(5,10,50,100)] for coeff in [0,1,2,5,10] for w in (5,10,50,100)],
            "spkt_welch_density": [{"coeff": coeff} for coeff in range(1,11)],
            "ar_coefficient": [{"coeff": coeff, "k": k} for coeff in [1, 2, 3, 4, 5] for k in [10,100,250,499]],
...
class SecondFCParameters(PickableSettings):
    """
    This class is a child class of the ComprehensiveFCParameters class
    and has the same functionality as its base class. The only difference is,
    that some of the feature calculators are not returned in the second FE stage
    (1h of TS from first FE stage).
    """
    ...
    def __init__(self):
        ...
        for name, func in feature_calculators.__dict__.items():
            if callable(func) and not hasattr(func, "not_in_second") and hasattr(func, "fctype"):
                name_to_param[name] = None
        ...
        name_to_param.update({
            ...
            "change_quantiles": [{"ql": ql, "qh": qh, "isabs": b, "f_agg": f}
                                          for ql in [0., .2, .4, .6, .8] for qh in [.2, .4, .6, .8, 1.]
                                          for b in [True] for f in ["mean", "var"] if ql < qh],
            "approximate_entropy": [{"m": 6, "r": r} for r in [.1, .3, .5, .7, .9]],
            "friedrich_coefficients": (lambda m: [{"coeff": coeff, "m": m, "r": 30} for coeff in range(m + 1)])(3),
            "max_langevin_fixed_point": [{"m": 3, "r": 30}],
            "linear_trend": [{"attr": "pvalue"}, {"attr": "rvalue"}, {"attr": "intercept"},
                             {"attr": "slope"}, {"attr": "stderr"}],
